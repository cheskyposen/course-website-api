...
class assignment {
    ...
    public function deleteAssignment($data){
        $this->db->query('DELETE FROM assignments WHERE teacher_id = :teacher_id AND asn_id = :asn_id');
        $this->db->bind(':teacher_id', $data['teacher_id']);
        $this->db->bind(':asn_id', $data['asn_id']);
    ...
    public function editAssignment($data){
        $this->db->query('UPDATE assignments SET asn_title = :asn_title, asn_body = :asn_body, asn_date_created = NOW(),  asn_due_date = :asn_due_date, asn_grade = :asn_grade WHERE teacher_id = :teacher_id AND asn_id = :asn_id');
        $this->db->bind(':teacher_id', $data['teacher_id']);
        $this->db->bind(':asn_id', $data['asn_id']);
        $this->db->bind(':asn_title', $data['asn_title']);
        $this->db->bind(':asn_body', $data['asn_body']);
